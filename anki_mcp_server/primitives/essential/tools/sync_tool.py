...
_DESCRIPTION = (
    ...
    "required (only when status=='conflict': the ChangesRequired name that "
    "forced the full sync; null otherwise), result (on success), error "
    "({code, message, category, hint} on error), job_id, started_at, "
    "finished_at.\n"
    "\n"
    "RESULT FIELDS (on status=='success'): result.outcome is the backend's "
    "post-sync state -- 'NO_CHANGES' (nothing to transfer) or 'NORMAL_SYNC' "
    "(changes were transferred); either way the sync is COMPLETE and nothing "
    "further is required. Do NOT confuse it with the top-level 'required' "
    "field, which is only populated for conflicts (null otherwise). On the "
    "resolve path result "
    "carries resolved:true instead of outcome. result.media_error is null "
    "when media synced fine, or an error payload when media failed -- media is "
    "best-effort and never fails the collection sync itself.\n"
    "\n"
