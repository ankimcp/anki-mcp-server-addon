...
Since the cache is no longer guaranteed to be deleted along with the addon folder, `native_cache_cleanup.py` registers `on_addons_dialog_will_delete_addons` on `gui_hooks.addons_dialog_will_delete_addons` to clean it up explicitly. That hook fires **only for deletion triggered from the Add-ons dialog** (uninstall); Anki's own update path (`download_addons` → `install` → `_install` → `deleteAddon`) never fires `addons_dialog_will_delete_addons`. This means an install currently running in fallback (in-folder cache) mode is still exposed to the original Windows update failure this whole relocation fixes — a known, currently-unaddressed gap, not a hard limitation of the approach. Anki does offer `gui_hooks.addon_manager_will_install_addon` (fires right before `_install`), which could relocate the legacy in-folder cache before an update; that's deliberately not implemented in this change. The registration itself lives in `__init__.py` **before** the `ensure_pydantic_core()` / `ensure_rpds()` gates on purpose: a gutted add-on (native extension deleted/moved mid-update on Windows) aborts import at one of those gates, and the hook needs to already exist at that point so the user's next uninstall/reinstall from the Add-ons dialog can still clean up the orphaned out-of-folder cache. It is **cleanup-only** and never touches the download/retry logic:
...
- The whole handler is wrapped so no exception can escape it: Anki drops a hook callback that raises (and never re-invokes it), so a raise here would look like nothing happened, and a re-raise would abort the user's delete/update entirely.

**Known residual gotcha**: the update TO the first version carrying this fix still fails once on Windows, because the OLD (already-loaded) code is the one holding the locked `.pyd` when the new files are unpacked. Verified workaround: disable the addon → restart Anki → Check for Updates → re-enable → restart again.
